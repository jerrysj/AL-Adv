...
from tools.final_util import set_random, NUM_POINTS, NUM_REGIONS
...
def save_fps(args):
    if args.dataset == "modelnet40":
        data_loader = DataLoader(ModelNet_Loader_Shapley_test(args, partition='train', num_points=args.num_points),num_workers=1,
    ...
    for data, label, normal in data_loader:
    ...
    fps_index_all = np.concatenate(fps_index_all) 
    print(fps_index_all)
    np.save('fps_%s_%d_%d_center_index.npy'%(args.dataset, args.num_points, args.num_regions), fps_index_all)
